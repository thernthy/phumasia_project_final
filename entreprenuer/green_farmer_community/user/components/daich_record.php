...
chickenChoose.addEventListener("change", function() {
  ...
  fetch(`http://localhost/Green_famer_project/user/routing/handle_chike_type_select.php?chicken_type=${choosedChiken}`)
    .then(response => {
...
document.getElementById('chiken_dai_record').addEventListener('submit', function(event) {
    ...
    xhr.open('POST', 'http://localhost/Green_famer_project/user/routing/get_chiken_dai_record_req.php', true);
    xhr.onload = function() {
        if (xhr.status === 200) {
