...
?>
            ...
            <div class="card_check">
                ...
                <div class="circle" id="b_ch_sold_block">
                    <button class="viw_sell_b_ch_btn" onclick="viewAllBadyChiken()">មើលបន្ថែម</button>
                                ...
                                while ($row = mysqli_fetch_assoc($view_each_bch_data)) {
                                    ...
                                    if($days_since_insert == 30) {
                                        if(!$get_chiken_avarible <=0){
                                            ...
                                            $update_bch_dai = "UPDATE bch_dai SET Status_dia ='active'  
                                    ...
                                    <td style="font-size:'.$font_size.'rem; color:'.(($vaccines_one!='')?'green':'red').'">
                                        ...
                                        '.(($reminder!='' && $reminder===$vaccines_one)?"<span style='color:yellow'>រំលឹក: ញូកាស</span>":'').'
                                    </td>
                                    <td style="font-size:'.$font_size.'rem;color:'.(($vaccines_two!='')? 'green':'red').';">
                                        '.(($fir_vaccinecheckdate->modify('+7 days')->Format('d/m/Y'))).'<br>
                                        '.(($reminder!='' && $reminder===$vaccines_two)?"<span style='color:yellow'>រំលឹក: កុំប៉ូរ៉ូ</span>":'').'
                                    </td>
                                    <td style="font-size:'.$font_size.'rem;color:'.(($vaccines_three!='')? 'green':'red').';">
                                        '.(($fir_vaccinecheckdate->modify('+7 days')->Format('d/m/Y'))).'<br>
                                        '.(($reminder!='' && $reminder===$vaccines_three)?"<span style='color:yellow'>រំលឹក: អ៊ុត</span>":'').'
                                    </td>
                                </tr>';
                                }
                            }
            ...
            <div class="viewAll_bch_container" id="viewAll_bch_container_sold">
                <div class="data_dable">
                    ...
                    <div style="position: absolute; bottom:10px; right:20px;">
                        ...
                        <h5>ចំនួនទឹកប្រាក់សរុប: <?php echo (($totle_b_ch_sold_price!=0)?'<span style="color:green;">$ '.$totle_b_ch_sold_price.'</span>':'<span style="color:red;">'.$totle_b_ch_sold_price.'</span>')?> </h5>
                    </div>
                </div>
            </div>
