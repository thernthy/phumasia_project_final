           ...
           if(isset($_POST['fine_barcode'])){
            ...
            if(!$view_chiken_data){
                ...
                while($row = mysqli_fetch_assoc($view_chiken_data)){
                    ...
                    $chiken_sold = $row['chiken_sold'];
                    $chiken_code = $row['chiken_barcode'];
                    $chiken_date  = $row['chiken_date'];
                    $chiken_qty = ($chiken_sold!=0)? $row['chiken_qty'] - $chike_sold_qty : $row['chiken_qty'];
                    $current_date = date('Y-m-d');
                    $total += $chiken_qty;
                    $days_since_insert = floor((strtotime($current_date) - strtotime($chiken_date)) / (60 * 60 * 24));
                    $check_data ++;
                    echo'
                    <tr>
                        <td>'.date('d-F-Y', strtotime($chiken_date)).'</td>
                        <td>'.$chiken_code.'</td>
                        <td>'.(($days_since_insert!=0)?$days_since_insert."<span style='color:green;'> ថ្ងៃ</span>" : '<span style="color:green;">ថ្ងៃនេះ</span>').'</td>
                        <td>'.$chiken_qty.'</td>
                    </tr>
                    ';
                }
    ...
    <div class="cardHeader" style="margin: 20px;">
            ...
            <button class="btn" style="margin:10px;" id="merl_btn" onclick="view_sold_ch_data()">មើលការលក់</button>
                ...
                <?php
                    ...
                    ch_qty_amount, ch_price, cmr_name, cmr_address, 
                        ...
                        while($row = mysqli_fetch_assoc($view_ch_sold_ifo)){
                            ...
                            echo'
                                <tr style="font-size:.8rem;">
                                 ...
                                 <td>'.$ch_sold_price.' $</td>
                                 <td>'.$total_amount.' $</td>
                                 <td>'.$ch_sold_cmr_name.'</td>
                                 <td>'.$ch_sold_cmr_add.'</td>
                                 <td>'.$ch_sold_cmr_contact_add.'</td>
                                 <td>'.$ch_sold_note.'</td>
                                </tr>
                    ...
                    echo"
                          ...
                          <span style='color:".(($total_ch_sold!=0)?'green':'red')."'>ចំនួនកូនមាន់លក់សរុប ".$total_ch_sold." ក្បាល</span><br>
                          <span style='color:".(($total_price!=0)?'green':'red')."'>ទឹកប្រាក់សរុប ".$total_price." $</span><br>
                          </div>
                        </div>
                        ";
                    if($check_data_sold === 0){
                        echo"
                            <h4 style='text-align:center; color:red;'>មិនមានការលក់</h4>
                        ";
                    }else{
                        echo"
